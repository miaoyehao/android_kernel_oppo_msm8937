...
static ssize_t tp_devices_check_read_func(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
    ...
    ret = snprintf(pagesize, sizeof(pagesize), "%d\n", ts->panel_data.tp_type);
...
//proc/touchpanel/Goodix/baseline
static int gt1x_tp_debug_read_func(struct seq_file *s, void *v, debug_type debug_type)
{
    ...
    if (!goodix_ops->goodix_debug_info_read) {
    ...
    goodix_ops->goodix_debug_info_read(s, ts->chip_data, debug_type);

    //soft reset, return to normal mode
...
static int proc_data_baseline_read(struct seq_file *s, void *v)
{
    gt1x_tp_debug_read_func(s, v, GTP_RAWDATA);
    return 0;
}
    

static int gt1x_data_baseline_open(struct inode *inode, struct file *file)
{
    return single_open(file, proc_data_baseline_read, PDE_DATA(inode));
}


static int proc_data_delta_read(struct seq_file *s, void *v)
{
    gt1x_tp_debug_read_func(s, v, GTP_DIFFDATA);
    return 0;
}

static int gt1x_data_delta_open(struct inode *inode, struct file *file)
{
    return single_open(file, proc_data_delta_read, PDE_DATA(inode));
}
...
static const struct file_operations gt1x_tp_baseline_data_proc_fops = {    
    .owner = THIS_MODULE, 
    .open = gt1x_data_baseline_open, 
    .read = seq_read, 
    .release = single_release, 
};

static const struct file_operations gt1x_tp_delta_data_proc_fops = {    
    .owner = THIS_MODULE, 
    .open = gt1x_data_delta_open, 
    .read = seq_read, 
    .release = single_release, 
};
...
int Goodix_create_proc(struct touchpanel_data *ts, struct goodix_proc_operations *goodix_ops)
{
    ...
    prEntry_tmp = proc_create_data("baseline", 0666, prEntry_gt, &gt1x_tp_baseline_data_proc_fops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
    ...
    //show baseline data interface
    prEntry_tmp = proc_create_data("delta", 0666, prEntry_gt, &gt1x_tp_delta_data_proc_fops, ts);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        TPD_INFO("%s: Couldn't create proc entry, %d\n", __func__, __LINE__);
    }
